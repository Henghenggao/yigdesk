...
def decision_dict(d) -> dict[str, Any]:
    return {
        ...
        "owner_identity": d.owner_identity,
        "cutoff": ({"seq": d.cutoff_seq, "actor": d.cutoff_actor,
                    "reason": d.cutoff_reason, "agent_identity": d.cutoff_identity}
                   if d.cutoff_seq is not None else None),
        "candidates": {cid: {"id": c.id, "author": c.author,
        ...
        "late_writes": [asdict(write) for write in d.late_writes],
