...
def _require_open_decision(txn, decision_id):
    decision = fold(txn.read()).decisions.get(decision_id)
    if decision is None:
...
class Blackboard:
    ...
    def open_decision(self, decision_id, question, decision_type, policy, *, actor, role):
        with self.ledger.transaction() as txn:
            if decision_id in fold(txn.read()).decisions:
    ...
    def propose_candidate(self, decision_id, candidate_id, action, *, actor, role):
        ...
        with self.ledger.transaction() as txn:
            _require_open_decision(txn, decision_id)
    ...
    def request_resolve(self, decision_id, *, actor, role):
        result, _replayed = self.request_resolve_with_status(
            decision_id, actor=actor, role=role
        )
        return result

    def request_resolve_with_status(self, decision_id, *, actor, role):
        with self.ledger.transaction() as txn:
            board = fold(txn.read())
